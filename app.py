...
def gen_frames_photo(img_file):
    ...
    # Initialize variables to store previous predictions
    prev_gender, prev_age = None, None
    ...
    for faceBox in faceBoxes:
        ...
        if prev_gender is not None and prev_age is not None:
            gender = (gender + prev_gender) / 2
            age = (age + prev_age) / 2

        # Store current predictions for future smoothing
        prev_gender, prev_age = gender, age

        # Show the output frame with smoothed predictions
        cv2.putText(resultImg, f'{gender}, {age}', (
            faceBox[0], faceBox[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2, cv2.LINE_AA)
